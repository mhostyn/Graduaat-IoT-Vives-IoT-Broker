...
for($t = 0; $t <= (count($AllSensorIds) -1); $t ++){ //3.4
    ...
    $selectLastTenQuery = 'SELECT D.value FROM data D, User U WHERE sensor_id = '. $lastTenString .' AND U.email = "'.$_SESSION["email"].'" AND U.password ="'.$_SESSION["password"].'" GROUP BY data_id LIMIT 10';
...
//|||||||
    //--|SET IDS FOR THE TABLE
    // $tableIds = "select data_id from data where sensor_id = {$lastTenString} LIMIT 10";
    $tableIds = 'select D.data_id from data D, User U WHERE sensor_id = '. $lastTenString .' AND U.email = "'.$_SESSION["email"].'" AND U.password="'.$_SESSION["password"].'" GROUP BY data_id LIMIT 10;';
    $idsToPush = $this->setValues($tableIds);

    // $this->tableIds = $this->setValues($tableIds);
...
$selectLastTenDataIDQuery = 'SELECT D.data_id FROM data D, User U WHERE U.email = "'.$_SESSION["email"].'" AND U.password="'.$_SESSION["password"].'" GROUP BY data_id LIMIT 10';
